...
def cluster_blocks(blocks: list[dict], line_spacing_threshold: float = 1.5, x_overlap_threshold: float = 0.5) -> list[dict]:
    """
    Cluster fragmented PDF text blocks into semantic paragraphs based on spatial proximity.
    
    Args:
        blocks: List of block dictionaries with 'x', 'y', 'width', 'height', 'slide_index'.
        line_spacing_threshold: Max vertical distance between blocks as a multiple of font size.
        x_overlap_threshold: Required horizontal overlap or alignment to consider merging.
        
    Returns:
        Clustered (merged) blocks.
    """
    if not blocks:
        return []
    ...
    for page_idx in sorted(pages.keys()):
        page_blocks = sorted(pages[page_idx], key=lambda b: (b['y'], b['x']))
        if not page_blocks:
            continue
        ...
        for next_block in page_blocks[1:]:
            ...
            if v_dist < (font_size * line_spacing_threshold) and v_dist > -5 and is_aligned:
                # Merge text
