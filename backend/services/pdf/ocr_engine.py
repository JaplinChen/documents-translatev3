...
from backend.services.extract_utils import is_numeric_only, is_garbage_text
...
def is_noisy_text(text: str) -> bool:
    ...
    if letters == 0 and sum(ch.isdigit() for ch in cleaned) == 0:
        return True
    other = len(cleaned) - letters - sum(ch.isdigit() for ch in cleaned) - sum(ch.isspace() for ch in cleaned)
    if other / total > 0.35:
        return True
...
def perform_ocr_on_page(pdf_path: str, page_index: int, config: dict | None = None) -> tuple[list[dict], float]:
    cfg = config or get_ocr_config()
    try:
        images = convert_from_path(
            pdf_path, first_page=page_index + 1, last_page=page_index + 1,
            dpi=cfg["dpi"], poppler_path=get_poppler_path()
        )
        if not images: return [], 0
        image = enhance_image_for_ocr(images[0])
        ocr_data = pytesseract.image_to_data(image, output_type=pytesseract.Output.DICT, lang=cfg["lang"], config=f"--psm {cfg['psm']}")
        
        line_map = {}
        for i in range(len(ocr_data["text"])):
            text = ocr_data["text"][i].strip()
            conf = int(ocr_data["conf"][i]) if ocr_data["conf"][i] != "-1" else -1
            if not text or conf < cfg["conf_min"] or is_numeric_only(text) or is_noisy_text(text) or is_garbage_text(text):
                continue
            key = (ocr_data["block_num"][i], ocr_data["line_num"][i])
            if key not in line_map:
                line_map[key] = {"text": [], "left": ocr_data["left"][i], "top": ocr_data["top"][i], 
                                 "right": ocr_data["left"][i] + ocr_data["width"][i], "bottom": ocr_data["top"][i] + ocr_data["height"][i]}
        ...
        for key, val in line_map.items():
            ...
            block = make_block(
                slide_index=page_index, shape_id=2000 + key[0] * 100 + key[1], block_type="pdf_text_block",
                source_text=full_text, x=val["left"] * scale, y=val["top"] * scale,
                width=(val["right"] - val["left"]) * scale, height=(val["bottom"] - val["top"]) * scale
            )
            block["is_ocr"] = True
            blocks.append(block)
...
def perform_paddle_ocr_on_page(pdf_path: str, page_index: int, config: dict | None = None) -> tuple[list[dict], float]:
    cfg = config or get_ocr_config()
    try:
        images = convert_from_path(
            pdf_path, first_page=page_index + 1, last_page=page_index + 1,
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            if not text or is_noisy_text(text) or is_garbage_text(text): continue
            block = make_block(
                slide_index=page_index, shape_id=4000 + idx, block_type="pdf_text_block",
                source_text=text, x=line["x0"] * scale, y=line["y0"] * scale,
                width=(line["x1"] - line["x0"]) * scale, height=(line["y1"] - line["y0"]) * scale
            )
            block["is_ocr"] = True
