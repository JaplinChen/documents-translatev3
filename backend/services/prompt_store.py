...
5. **語言與標點**：必須符合目標語言當地的標點符號規範與技術習慣。
6. **雙語保留原則 (Bilingual Preservation)**：若原文區塊同時包含第一語言(如越文)與第二語言(如中文)，在翻譯或「校正模式」下，你**必須字節不差地在輸出中保留原文的第一語言部分**，僅對第二語言進行校正或翻譯。輸出應呈現為「原文第一語言 + 修訂後第二語言」的格式。
