...
## 提取準則
1. **核心價值**：只提取真正具備專業定義、穩定翻譯且對文件一致性至關重要的核心術語、名詞或技術縮寫。
2. **過濾無效項**：**嚴禁提取**純型號 (如 SFG16-72)、序列號、數值、日期或兩語言中完全相同的純程式碼。
3. **禁止提取描述詞 (描述性質短句)**：
    - **禁止**：行為描述（如：處理效能下降、錄製效果差）。
    - **禁止**：狀態描述（如：品質優良、不一致的品質、參差不齊）。
    - **禁止**：商業/成本描述（如：昂貴的製作成本、外包費用）。
    - **禁止**：通用動詞短語（如：更新系統版本、建議修正內容）。
4. **正確提取範例**：
    - **實體產品**：Intel Core i3, Uipath Studio (RPA)。
    - **專業概念**：RPA (機器人流程自動化), 邊緣運算, 同發併發數。
5. **智慧糾錯 (Typo Handling)**：若發現原文有拼錯（如 'Lapop'），請在 `target` 提供正確譯文。
6. **翻譯精準**：提供最符合專業語境的目標語言 {target_language} 譯文。
7. **因果說明**：簡短說明為何提取此術語。
