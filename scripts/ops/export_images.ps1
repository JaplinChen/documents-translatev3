...
Copy-Item (Join-Path $ProjectRoot.FullName "scripts\\ops\\install.bat") (Join-Path $ExportDir "install.bat")
if (!(Test-Path (Join-Path $ExportDir "scripts"))) { New-Item -ItemType Directory -Path (Join-Path $ExportDir "scripts") }
Copy-Item (Join-Path $ProjectRoot.FullName "scripts\\ops\\setup.ps1") (Join-Path $ExportDir "scripts\\setup.ps1")
